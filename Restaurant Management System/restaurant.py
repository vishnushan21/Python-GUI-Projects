...
def Send():
    ...
    frame5 = Frame(root, width=300, height=340, relief=RIDGE, borderwidth=5, bg='#248aa2', 
                   highlightbackground="white", highlightcolor="white", highlightthickness=2)
    ...
    l3 = LabelFrame(innerframe5, text="Send Bill Through Email", width=270, height=310, borderwidth=3, 
                    font=('verdana', 10, 'bold'), fg='#248aa2', relief=RIDGE, 
                    highlightbackground="white", highlightcolor="white", highlightthickness=2)
    l3.place(x=2, y=2)

    l4 = Label(innerframe5, text="Recipient Email", font=('verdana', 10, 'bold'))
    l4.place(x=40, y=40)

    email_entry = Entry(innerframe5, width=30, borderwidth=2)
    email_entry.place(x=40, y=70)
    ...
    #b_detail.insert(1.0, "Enter bill details here...")

    def send_bill():
        recipient_email = email_entry.get()
        messages = b_detail.get("1.0", "end-1c")
        total_cost = total_cost_cost.get()
        

        if recipient_email == "":
            messagebox.showerror("Error", 'Please fill in the recipient email address')
        elif messages == "":
            messagebox.showerror("Error", 'Bill Details are empty')
        else:
            try:
                # SMTP configuration
                sender_email = "vishnushankar950@gmail.com"  # Replace with your email
                sender_password = "sxukaqktgiwpjxez"  # Replace with your password
                ...
                messagebox.showinfo("Send Email", 'Bill has been sent successfully')
...
l1 = Label(frame,text="      VISHNU'S RESTAURANT      ",font=('roboto',30,'bold'),bg='#248aa2',fg="#ffffff")
l1.place(x=10,y=4)
